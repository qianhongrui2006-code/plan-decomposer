...
6. 任务量必须铺满整个周期：总任务数 8-12 个（周期 ≥10 天取 10-12 个；4-9 天取 7-9 个；≤3 天的短周期 5-6 个即可）。每个里程碑至少 3 个任务；里程碑 2-3 个，按时间先后排列。
7. 天数必须连续覆盖：里程碑 day_range 横跨 N 天时，其下任务的 day 要逐天铺开，相邻任务的 day 间隔不得超过 1 天；禁止出现「某阶段标称 5 天却只有 1-2 个任务」的情况。
8. 每日负荷接近日投入能力：每天的任务总时长 ≈ 日投入能力的 70%-130%（用户未给出日投入时按每天 2 小时估算）；整个周期的任务总时长 ≈ 天数 × 日投入能力，不允许整个周期只有几个小时的工作量。
