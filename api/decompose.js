...
6. 【硬性约束：每一天都必须有任务】总任务数必须等于计划总天数（用户说"两周"就生成14个任务，每天1个；说"一周"就生成7个）。不允许出现任何空白天。如果某个任务需要多天完成，就把它拆成多个子任务（如"第3天上半""第3天下半"），每天一个条目，day 字段逐天递增。
7. 【硬性约束：每天时长必须填满】每天的任务总时长必须达到日投入能力的 90%-110%。举例：用户日投入 3 小时（180 分钟），则每一天的任务 duration_minutes 之和必须 ≈ 180 分钟（允许 160-200 分钟浮动）。禁止出现"日投入3小时却只排1小时任务"的情况。如果单个任务撑不满一天，就在同一天安排 2-3 个不同主题的任务。
8. 【硬性约束：天数逐日递增无跳号】day 字段必须从 1 开始连续递增到总天数，不能跳号（不能有第4天之后直接第6天）。相邻任务的 day 差值只能是 0（同一天多任务）或 1（下一天）。
